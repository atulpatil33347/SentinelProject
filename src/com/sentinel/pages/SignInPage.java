...
public class SignInPage extends WebDriverTestBase {
			...
			case "verifyMaxLimitValidUsernameInvalidPassword":
				...
				while (intValue >= 0) {
					...
					if (intValue != 0) {
						...
						SoftAssertUtil.myAssert().assertTrue(getToasterMessage().getText().equals(
								"Your account is inactivated due to 5 incorrect attempt in single day.Please Contact admin for further assistance."),
	...
	public void verifyBlankEmailAddressAndMobileNumErrorMessages() {
		try {
			...
			SoftAssertUtil.myAssert()
					.assertTrue((getErrorMessageBlankMobileNumber().isDisplayed())
	...
	public String getvalueFromMailBox(String... args) {
		try {
			...
			if (args.length > 1) {
				...
				/**
				 * Close the unwanted windows and focus on the forget password window
				 */
	...
	public void loginApplication(String... args) {
		try {
			...
			genericFunctions.logEvent("Logged in the Application");
		} catch (Exception e) {
			e.printStackTrace();
		}
